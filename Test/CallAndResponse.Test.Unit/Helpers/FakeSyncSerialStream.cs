...
/// <summary>
/// A <see cref="Stream"/> stand-in for an open <c>System.IO.Ports</c> serial port, used to
/// drive <c>BclSerialDuplexPipe</c>'s synchronous read pump without hardware.
/// </summary>
/// <remarks>
/// <para>
/// The BCL pump calls the blocking <see cref="Read(byte[], int, int)"/>, not
/// <c>ReadAsync</c>, so <see cref="FakeSerialStream"/> cannot drive it. Scripted steps are
/// played in order: a step either hands back bytes or throws. Once they run out the stream
/// behaves like an idle port — each read waits briefly and then throws
/// <see cref="TimeoutException"/>, the way an expired <c>ReadTimeout</c> does.
/// </para>
/// <para>
