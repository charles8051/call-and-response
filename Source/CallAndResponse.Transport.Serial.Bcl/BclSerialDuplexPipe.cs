...
/// <summary>
/// An <see cref="IDuplexPipe"/> backed by an already-open <see cref="SerialPort"/>.
/// The caller owns the serial port lifecycle (open/close/dispose).
/// </summary>
/// <remarks>
/// <para>
/// The read pump is synchronous, on a dedicated thread, and that is not a style choice. On
/// Windows <c>SerialPort.BaseStream.ReadAsync</c> honours neither cancellation nor a timeout:
/// it ignores the <see cref="CancellationToken"/> (dotnet/runtime#30850), and
/// <see cref="SerialPort.ReadTimeout"/> is documented as not affecting <c>BeginRead</c>, which
/// is what <c>ReadAsync</c> is built on. A read issued that way returns only when bytes arrive
/// or the port closes.
/// </para>
/// <para>
...
public sealed class BclSerialDuplexPipe : IDuplexPipe, IAsyncDisposable
{
    ...
    /// spins, the pipe never faults, and the consumer hangs until its own token fires. See
    /// ADR-0019 DEC-009b.
    /// </remarks>
    ...
    /// close-to-unblock pattern this library removed. Consumers are unaffected either way:
    /// they wait on <see cref="Input"/>, which cancels immediately.
    /// </remarks>
